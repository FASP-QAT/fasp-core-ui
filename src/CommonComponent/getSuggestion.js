...
export default function getSuggestion(row, lang) {
    ...
    if (row.realmProblem.problem.problemId == 3) {
        ...
        if (desc_en != null && desc_en != '') {
            const result_en = desc_en.split('<%SHIPMENT_ID%>').join(row.shipmentId).split('<%PLANNING_UNIT%>').join(getLabelText(row.planningUnit.label, lang)).split('<%PROCUREMNET_AGENT%>').join(row.data5 != "" ? obj.procurementAgentCode : "").split('<%RO_NO%>').join(row.data5 != "" ? obj.orderNo : "").split('<%SHIPMENT_QTY%>').join(row.data5 != "" ? obj.shipmentQuantity : "").split('<%SHIPMENT_DATE%>').join(row.data5 != "" ? moment(obj.shipmentDate).format("MMM-DD, YYYY") : "");
            label.label_en = result_en;
        } if (desc_fr != null && desc_fr != '') {
