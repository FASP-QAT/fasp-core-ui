...
export default function getSuggestion(row, lang) {
    ...
    if (row.realmProblem.problem.problemId == 3) {
        ...
        if (desc_en != null && desc_en != '') {
            const result_en = desc_en.split('<%SHIPMENT_ID%>').join(row.shipmentId).split('<%PLANNING_UNIT%>').join(getLabelText(row.planningUnit.label, lang)).split('<%PROCUREMNET_AGENT%>').join(obj.procurementAgentCode).split('<%RO_NO%>').join(obj.orderNo).split('<%SHIPMENT_QTY%>').join(obj.shipmentQuantity).split('<%SHIPMENT_DATE%>').join(obj.shipmentDate);
            label.label_en = result_en;
        } if (desc_fr != null && desc_fr != '') {
            const result_fr = desc_fr.split('<%SHIPMENT_ID%>').join(row.shipmentId).split('<%PLANNING_UNIT%>').join(getLabelText(row.planningUnit.label, lang)).split('<%PROCUREMNET_AGENT%>').join(obj.procurementAgentCode).split('<%RO_NO%>').join(obj.orderNo).split('<%SHIPMENT_QTY%>').join(obj.shipmentQuantity).split('<%SHIPMENT_DATE%>').join(obj.shipmentDate);
            label.label_fr = result_fr;
        } if (desc_sp != null && desc_sp != '') {
            const result_sp = desc_sp.split('<%SHIPMENT_ID%>').join(row.shipmentId).split('<%PLANNING_UNIT%>').join(getLabelText(row.planningUnit.label, lang)).split('<%PROCUREMNET_AGENT%>').join(obj.procurementAgentCode).split('<%RO_NO%>').join(obj.orderNo).split('<%SHIPMENT_QTY%>').join(obj.shipmentQuantity).split('<%SHIPMENT_DATE%>').join(obj.shipmentDate);
            label.label_sp = result_sp;
        } if (desc_pr != null && desc_pr != '') {
            const result_pr = desc_pr.split('<%SHIPMENT_ID%>').join(row.shipmentId).split('<%PLANNING_UNIT%>').join(getLabelText(row.planningUnit.label, lang)).split('<%PROCUREMNET_AGENT%>').join(obj.procurementAgentCode).split('<%RO_NO%>').join(obj.orderNo).split('<%SHIPMENT_QTY%>').join(obj.shipmentQuantity).split('<%SHIPMENT_DATE%>').join(obj.shipmentDate);
            label.label_pr = result_pr;
        }
