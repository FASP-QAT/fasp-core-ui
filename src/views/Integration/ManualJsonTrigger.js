...
export default class ConsumptionDetails extends Component {
            ...
            loading: true
                    ...
                    IntegrationService.getIntegrationListAll().then(response => {
                        if (response.status == 200) {
                            ...
                            var integrationList = [];
                            listArray.map(item => {
                                integrationList.push({ id: item.integrationId, name: (item.integrationName).toUpperCase() })
                            })
                            ...
                            RealmCountryService.getRealmCountryForProgram(realmId)
                                .then(countryResponse => {
                                    var countryValues = [];
                                    var listArray = countryResponse.data.map(ele => ele.realmCountry);
                                    listArray.map(ele1 => {
                                        countryValues.push({ label: getLabelText(ele1.label, this.state.lang), value: ele1.id })
                                    })
                                    listArray.sort((a, b) => {
    ...
    filterVersion = function (instance, cell, c, r, source) {
        ...
        var versionList = this.state.programList.filter(c => c.id == value)[0].versionList;
        var vlList = [];
        versionList.map(item => {
            var name = ((item.versionStatus.id == 2 && item.versionType.id == 2) ? item.versionId + '*' : item.versionId) + " (" + moment(item.createdDate).format(`MMM DD YYYY`) + ")";
...
//to get values
            ...
            .then(response => {
                ...
                if (response.status == "200") {
                    ...
                    for (var i = 0; i < dataForJexcel.length; i++) {
                        ...
                        data[5] = dataForJexcel[i].completedDate != null ? i18n.t('static.manualIntegration.completed') : i18n.t('static.manualIntegration.requested');
                        tableData.push(data);
                    }
                    ...
                    var varEL = ""
                    this.el = jexcel(document.getElementById("tableDivReport"), options);
                    varEL = this.el
                    this.setState({
                        // dataEL: varEL,
                        loading: false,
    ...
    render() {
        ...
        let programList = [];
        programList = programListBasedOnCountry.length > 0
                ...
                <div>
                    <Card>
                        <div className="Card-header-reporticon">
                            <Button color="info" size="md" className="float-right mr-1" type="button" onClick={() => this.modelOpenClose()} disabled={this.state.loading ? true : false}>{i18n.t('static.manualIntegration.addManualIntegration')}</Button>
                        </div>
                        <CardBody className="p-0">
                            <Col xs="12" sm="12">
                                ...
                                <div className="">
                                    <div id="tableDivReport" style={{ display: this.state.loading ? "none" : "block" }} />
                                </div>
                    ...
                    <Modal isOpen={this.state.isModalOpen}
                        ...
                        <ModalHeader
                            ...
                            <strong>{i18n.t('static.manualIntegration.addManualIntegration')}</strong>
                        </ModalHeader>
                        <ModalBody>
                            <div className="">
                                <h5 className={this.state.color} id="div2">{i18n.t(this.state.messageModal)}</h5>
