...
import {
    ...
    CardBody,
    Col,
    Table, FormGroup, Input, InputGroup, Label, Form, Button, ModalHeader, ModalBody, Modal, CardFooter} from 'reactstrap';
...
import { SECRET_KEY, INDEXED_DB_VERSION, INDEXED_DB_NAME, REPORT_DATEPICKER_START_MONTH, REPORT_DATEPICKER_END_MONTH, TITLE_FONT, API_URL, PROGRAM_TYPE_DATASET } from '../../Constants.js'
                        ...
                        this.setState({
                            ...
                            this.setState({
                                ...
                                if (displayId == 2) {
                                    ...
                                    columns.push({ title: i18n.t('static.forecastOutput.totalForecastQuantity'), type: 'numeric', textEditor: true, mask: '#,##.00', decimal: '.', width: 100, readOnly: true });
                                    columns.push({ title: 'forecast Blank', type: 'hidden', width: 100, readOnly: true });
                                    let nestedHeaders = [];
                                    nestedHeaders.push(
                                        {
                                            title: '',
                ...
                ReportService.forecastSummary(inputJson)
                    .then(response => {
                        ...
                        if (displayId == 2) {
                            ...
                            columns.push({ title: i18n.t('static.product.product'), type: 'text', width: 100, readOnly: true });
                            for (var k = 0; k < uniqueRegionList.length; k++) {
                                columns.push({ title: i18n.t('static.compareVersion.selectedForecast'), type: 'text', width: 100, readOnly: true });
                                columns.push({ title: i18n.t('static.forecastReport.forecastQuantity'), type: 'numeric', textEditor: true, mask: '#,##.00', decimal: '.', width: 100, readOnly: true });
                                columns.push({ title: i18n.t('static.program.notes'), type: 'text', width: 100, readOnly: true });
                            }
                            columns.push({ title: i18n.t('static.forecastOutput.totalForecastQuantity'), type: 'numeric', textEditor: true, mask: '#,##.00', decimal: '.', width: 100, readOnly: true });
                            let nestedHeaders = [];
                            nestedHeaders.push(
