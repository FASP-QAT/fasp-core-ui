                                            ...
                                            if (checkIfThereIsUnMappedBudget == 0) {
                                              ...
                                              for (var cd = 0; cd < mergedConsumptionData.length; cd++) {
                                                ...
                                                if (oldDataList.length > 0) {
                                                  ...
                                                  oldData = [oldDataList[0].consumptionId, oldDataList[0].planningUnit.id, moment(oldDataList[0].consumptionDate).format(DATE_FORMAT_CAP_WITHOUT_DATE), oldDataList[0].region.id, (oldDataList[0].actualFlag.toString() == "true" ? 1 : 2), oldDataList[0].dataSource.id, oldDataList[0].realmCountryPlanningUnit.id, Math.round(oldDataList[0].consumptionRcpuQty), (rcpuForTableOldData[0].conversionMethod == 1 ? "*" : "/") + rcpuForTableOldData[0].conversionNumber.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ","), (Math.round(oldDataList[0].consumptionRcpuQty) * oldDataList[0].multiplier), oldDataList[0].dayOfStockOut, oldDataList[0].notes, oldDataList[0].active, JSON.stringify(oldDataList[0].batchInfoList != "" ? ((oldDataList[0].batchInfoList).map(function (a) { return { "batchNo": a.batch.batchNo, "qty": parseInt(a.consumptionQty) } })).sort(function (a, b) { return a.qty - b.qty; }) : ""), "", "", "", "", 4];
                                                }
                                                data[15] = oldData;
