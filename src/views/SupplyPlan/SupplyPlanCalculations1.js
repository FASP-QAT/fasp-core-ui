                            ...
                            for (var ppL = 0; ppL < programPlanningUnitList.length; ppL++) {
                                ...
                                for (var i = 0; createdDate < lastDataEntryDate; i++) {
                                    ...
                                    if (nationalAdjustment != 0) {
                                        var nationAdjustmentIndex = inventoryListForNationalAdjustments.findIndex(c => c.region == null && moment(c.inventoryDate).format("YYYY-MM") == moment(endDate).format("YYYY-MM") && c.planningUnit.id == programPlanningUnitList[ppL].planningUnit.id);
                                ...
                                if (page == "consumption") {
                                    ...
                                    ProgramService.saveProgramData(programJsonForStoringTheResult).then(response => {
                                        if (response.status == 200) {
                                            console.log("In save after success")
