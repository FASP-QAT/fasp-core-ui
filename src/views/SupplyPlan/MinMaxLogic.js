    ...
    openRequest.onsuccess = function (e) {
        ...
        programRequest.onsuccess = function (e) {
            ...
            for (var reg = 0; reg < regionListFiltered.length; reg++) {
                var c = consumptionList.filter(c => (c.consumptionDate >= mStartDate && c.consumptionDate <= mEndDate) && c.region.id == regionListFiltered[reg].id);
            ...
            var adjustmentQty = 0;
            for (var reg = 0; reg < regionListFiltered.length; reg++) {
                var c = inventoryList.filter(c => (c.inventoryDate >= mStartDate && c.inventoryDate <= mEndDate) && c.region != null && c.region.id == regionListFiltered[reg].id);
                for (var j = 0; j < c.length; j++) {
