...
export default class ConsumptionInSupplyPlanComponent extends React.Component {
            ...
            rcpuRequest.onsuccess = function (event) {
                ...
                dataSourceRequest.onsuccess = function (event) {
                    ...
                    var options = {
                        ...
                        onbeforechange: this.beforeChanged,
                        text: {
                                        ...
                                        var options = {
                                            ...
                                            contextMenu: function (obj, x, y, e) {
                                                ...
                                                if (y == null) {
                                                    ...
                                                    if (consumptionBatchEditable && obj.options.allowDeleteRow == true && obj.getJson().length > 1) {
                                                        // region id
    ...
    beforeChanged(instance, cell, x, y, value) {
        if (x == 12) {
            console.log("---------------->X12 is changing for y", y);
        }
        if(x==7){
            console.log("---------------->X6 is changing for y", y);
        }
    }
    ...
    consumptionChanged = function (instance, cell, x, y, value) {
        ...
        if (x != 12 && x != 13 && rowData[12] == "") {
            console.log("In if-------------->", rowData[12], "X---------->", x, "Y-------->", y);
            rowData[12] = -1;
            elInstance.setValueFromCoords(12, y, -1, true);
            elInstance.setValueFromCoords(13, y, 1, true);
            elInstance.setValueFromCoords(7, y, `=F${parseInt(y + 1)}*G${parseInt(y + 1)}`, true)
        }
        if (x == 0) {
        ...
        if (x == 5) {
            var valid = checkValidtion("number", "F", y, rowData[5], elInstance, JEXCEL_INTEGER_REGEX, 1, 1);
            if (valid == true) {
    ...
    checkValidationConsumptionBatchInfo() {
        ...
        for (var y = 0; y < json.length; y++) {
            ...
            if (checkDuplicateInMap.length > 1) {
                ...
                validation = checkValidtion("number", "C", y, rowData[2], elInstance, JEXCEL_INTEGER_REGEX, 1, 0);
                if (validation == false) {
                    valid = false;
                }
            }
    ...
    checkValidationConsumption() {
        ...
        for (var y = 0; y < json.length; y++) {
            console.log("y---->", y);
            var rd = elInstance.getRowData(y);
            console.log("Rd-------------->", rd);
            var map = new Map(Object.entries(rd));
            console.log("Map----------------->", map);
            console.log("Moment consumption date", moment(map.get("0")).format("YYYY-MM"));
            mapArray.push(map);
            ...
            if (checkDuplicateInMap.length > 1 || (checkDuplicateOverAll.length > 0 && index != map.get("12"))) {
                ...
                validation = checkValidtion("number", "F", y, rowData[5], elInstance, JEXCEL_INTEGER_REGEX, 1, 1);
                if (validation == false) {
                    valid = false;
                    // elInstance.setValueFromCoords(14, y, 1, true);
                } else {
                ...
                validation = checkValidtion("numberNotRequired", "I", y, rowData[8], elInstance, JEXCEL_INTEGER_REGEX, 1, 1);
                if (validation == false) {
                    valid = false;
                    // elInstance.setValueFromCoords(14, y, 1, true);
                } else {
        ...
        if (validation == true) {
            ...
            openRequest.onsuccess = function (e) {
                ...
                programRequest.onsuccess = function (event) {
                    ...
                    for (var i = 0; i < json.length; i++) {
                        var rd = elInstance.getRowData(i);
                        var map = new Map(Object.entries(rd));
                        if (map.get("13") == 1) {
                        ...
                        if (parseInt(map.get("12")) != -1) {
                            ...
                            console.log("elInstance.getCell(`H${parseInt(i) + 1}`)", elInstance.getCell(`H${parseInt(i) + 1}`));
