...
export default class ConsumptionInSupplyPlanComponent extends React.Component {
                                ...
                                for (var j = 0; j < jsonLength; j++) {
                                    try {
                                        ...
                                        if (rowData[12] != -1 && rowData[12] !== "" && rowData[12] != undefined) {
                                            ...
                                            if (rowData[12] != -1 && moment(rowData[0]).format("YYYY-MM") < moment(lastEditableDate).format("YYYY-MM-DD") && !AuthenticationService.checkUserACL([(document.getElementById("programId").value).toString().split("_")[0].toString()], "ROLE_BF_READONLY_ACCESS_REALM_ADMIN")) {
                                                if (rowData[15] > 0) {
    ...
    batchDetailsClicked(obj, x, y, e, consumptionEditable) {
        ...
        var lastEditableDate = "";
        lastEditableDate = moment(Date.now()).subtract(this.state.realm.actualConsumptionMonthsInPast + 1, 'months').format("YYYY-MM-DD");
        if (rowData[15] > 0 && moment(rowData[0]).format("YYYY-MM") < moment(lastEditableDate).format("YYYY-MM-DD") && rowData[12] != -1 && !AuthenticationService.checkUserACL([(document.getElementById("programId").value).toString().split("_")[0].toString()], "ROLE_BF_READONLY_ACCESS_REALM_ADMIN")) {
    ...
    batchInfoChangedConsumption = function (instance, cell, x, y, value) {
        ...
        if (x == 0) {
            ...
            if (valid == true) {
                if (value != -1) {
                    var expiryDate = this.props.items.batchInfoList.filter(c => (c.batchNo == (elInstance.getValue(`A${parseInt(y) + 1}`, false)).split("~")[0] && moment(c.expiryDate).format("YYYY-MM") == moment((elInstance.getValue(`A${parseInt(y) + 1}`, false)).split("~")[1]).format("YYYY-MM")))[0].expiryDate;
