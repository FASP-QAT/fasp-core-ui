...
export default class ConsumptionInSupplyPlanComponent extends React.Component {
                ...
                rcpuRequest.onsuccess = function (event) {
                    ...
                    dataSourceRequest.onsuccess = function (event) {
                        ...
                        for (var j = 0; j < consumptionList.length; j++) {
                            ...
                            data[17] = consumptionList[j].multiplier;
                        ...
                        if (consumptionList.length == 0 && consumptionEditable) {
                            ...
                            data[15] = 0;
                            data[16] = 0;
                            data[17] = realmCountryPlanningUnitList.length == 1 ? realmCountryPlanningUnitList[0].multiplier : "";;
                        ...
                        var options = {
                            ...
                            columns: [
                                ...
                                { title: i18n.t('static.supplyPlan.quantityPU'), type: 'numeric', mask: (localStorage.getItem("roundingEnabled") != undefined && localStorage.getItem("roundingEnabled").toString() == "false") ? '#,##.00' : '#,##', decimal: '.', width: 120, readOnly: true },
                                ...
                                for (var j = 0; j < jsonLength; j++) {
                                    try {
                                        ...
                                        if (rowData[13] != -1 && rowData[13] !== "" && rowData[13] != undefined) {
                                            ...
                                            if (rowData[13] != -1 && moment(rowData[1]).format("YYYY-MM") < moment(lastEditableDate).format("YYYY-MM-DD") && !AuthenticationService.checkUserACL([(document.getElementById("programId").value).toString().split("_")[0].toString()], "ROLE_BF_READONLY_ACCESS_REALM_ADMIN")) {
                                                if (rowData[16] > 0) {
    ...
    batchDetailsClicked(obj, x, y, e, consumptionEditable) {
        ...
        var lastEditableDate = "";
        lastEditableDate = moment(Date.now()).subtract(this.state.realm.actualConsumptionMonthsInPast + 1, 'months').format("YYYY-MM-DD");
        if (moment(rowData[1]).format("YYYY-MM") < moment(lastEditableDate).format("YYYY-MM-DD") && rowData[13] != -1 && !AuthenticationService.checkUserACL([(document.getElementById("programId").value).toString().split("_")[0].toString()], "ROLE_BF_READONLY_ACCESS_REALM_ADMIN")) {
    ...
    consumptionChanged = function (instance, cell, x, y, value) {
        ...
        if (x == 0) {
            ...
            if (valid == false) {
                ...
                if (realmCountryPlanningUnitList.length == 1) {
                    ...
                    elInstance.setValueFromCoords(7, y, (realmCountryPlanningUnitList[0].conversionMethod == 1 ? "*" : "/") + realmCountryPlanningUnitList[0].conversionNumber.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ","), true);
                    elInstance.setValueFromCoords(17, y, realmCountryPlanningUnitList[0].multiplier, true);
                }
            }
        }
        ...
        if (x == 5) {
            ...
            var valid = checkValidtion("text", "F", y, rowData[5], elInstance);
            if (valid == true) {
                var rcpuForTable = (this.state.realmCountryPlanningUnitList.filter(c => c.id == rowData[5].toString().split(";")[0])[0]);
                ...
                elInstance.setValueFromCoords(17, y, rcpuForTable.multiplier, true);
            }
        }
        if (x == 6) {
