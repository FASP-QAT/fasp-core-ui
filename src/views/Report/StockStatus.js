...
import { SECRET_KEY, FIRST_DATA_ENTRY_DATE, INDEXED_DB_NAME, INDEXED_DB_VERSION, DATE_FORMAT_CAP, REPORT_DATEPICKER_START_MONTH, REPORT_DATEPICKER_END_MONTH, API_URL } from '../../Constants.js'
...
class StockStatus extends Component {
  ...
  exportCSV() {
    ...
    (i18n.t('static.shipment.qty') + " | " + i18n.t('static.budget.fundingsource') + " | " + i18n.t('static.supplyPlan.shipmentStatus')).replaceAll(' ', '%20') + " | " + (i18n.t('static.report.procurementAgentName')) + " | " + (i18n.t('static.manualTagging.procOrderNo')),
    ...
    (ele.shipmentInfo.map(item => {
      return (
        item.shipmentQty + " | " + item.fundingSource.code + " | " + getLabelText(item.shipmentStatus.label, this.state.lang) + " | " + item.procurementAgent.code + " | " + (item.orderNo == null ? 'N/A' : item.orderNo)
      )
    }).join(' \n')).replaceAll(' ', '%20')
    ...
    list.map(
      (item) => {
        ...
        (ele.shipmentInfo.map(item1 => {
          return (
            item1.shipmentQty + " | " + item1.fundingSource.code + " | " + getLabelText(item1.shipmentStatus.label, this.state.lang) + " | " + item1.procurementAgent.code + " | " + (item1.orderNo == null ? 'N/A' : item1.orderNo)
          )
        }).join(' \n')).replaceAll(' ', '%20')
  ...
  exportPDF = () => {
    ...
    [
      ...
      (i18n.t('static.supplyPlan.qty') + " | " + i18n.t('static.supplyPlan.funding') + " | " + i18n.t('static.shipmentDataEntry.shipmentStatus') + " | " + (i18n.t('static.supplyPlan.procAgent')) + " | " + (i18n.t('static.manualTagging.procOrderNo'))),
                  ...
                  {this.state.show && this.state.stockStatusList.length > 0 && <Table responsive className="table-striped table-bordered text-center mt-2">
                    ...
                    <thead>
                      ...
                      <tr>
                        ...
                        <th className="text-center" style={{ width: "600px" }}>{i18n.t('static.report.qty') + " | " + (i18n.t('static.budget.fundingsource') + " | " + i18n.t('static.supplyPlan.shipmentStatus') + " | " + (i18n.t('static.report.procurementAgentName')) + " | " + (i18n.t('static.manualTagging.procOrderNo')))}</th>
                          ...
                          <tr id="addr0" key={idx} >
                            ...
                            <td align="center"><table >
                              {this.state.stockStatusList[idx].shipmentInfo.map((item, index) => {
                                return (<tr  ><td padding="0">{this.formatter(item.shipmentQty) + `   |    ${item.fundingSource.code}    |    ${item.shipmentStatus.label.label_en}   |    ${item.procurementAgent.code}   |    ${item.orderNo == null ? 'N/A' : item.orderNo}`}</td></tr>)
