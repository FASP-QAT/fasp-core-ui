            ...
            if (this.state.isAggregate.toString() == "true") {
              ...
              for (let m = moment(startDate).format("YYYY-MM"); moment(m).format("YYYY-MM") <= moment(stopDate).format("YYYY-MM"); m = moment(m).add(1, "months").format("YYYY-MM")) {
                ...
                stockStatusVerticalAggregateList.push({
                  ...
                  "mos": sp.reduce((sum, value) => sum + value.amc, 0) > 0 ? sp.reduce((sum, value) => sum + value.closingBalance, 0) / sp.reduce((sum, value) => sum + value.amc, 0) : null,
                  "minStockMos": ppu.length > 0 ? ppu.reduce((sum, value) => sum + value.minMonthsOfStock, 0) / ppu.length : 0,
                  "maxStockMos": ppu.length > 0 ? Number(ppu.reduce((sum, value) => sum + value.minMonthsOfStock, 0) / ppu.length) + Number(ppu.reduce((sum, value) => sum + value.reorderFrequencyInMonths, 0) / ppu.length) : 0,
                  "minStockQty": ppu.length > 0 ? ppu.reduce((sum, value) => sum + value.minQty, 0) / ppu.length : 0,
                  "maxStockQty": ppu.length > 0 ? Number(sp.reduce((sum, value) => sum + roundARU(value.amc, multiplier), 0)) * Number(Number(ppu.reduce((sum, value) => sum + value.minMonthsOfStock, 0) / ppu.length) + Number(ppu.reduce((sum, value) => sum + value.reorderFrequencyInMonths, 0) / ppu.length)) : 0,
                  ...
                  stockStatusVertical.push(
                    {
                      ...
                      "amc": sp.reduce((sum, value) => sum + roundARUWithoutRounding(value.amc, multiplier), 0),
                      "mos": sp.reduce((sum, value) => sum + value.amc, 0) > 0 ? sp.reduce((sum, value) => sum + value.closingBalance, 0) / sp.reduce((sum, value) => sum + value.amc, 0) : null,
                      "minMos": ppu.length > 0 ? ppu[0].minMonthsOfStock : null,
                      "maxMos": ppu.length > 0 ? Number(ppu[0].minMonthsOfStock) + Number(ppu[0].reorderFrequencyInMonths) : null,
                      "unmetDemand": sp.reduce((sum, value) => sum + roundARUWithoutRounding(value.unmetDemand, multiplier), 0),
                      "regionCount": sp[0].regionCount,
                      "regionCountForStock": sp[0].regionCountForStock,
                      "nationalAdjustment": sp.reduce((sum, value) => sum + roundARUWithoutRounding(value.nationalAdjustment, multiplier), 0),
