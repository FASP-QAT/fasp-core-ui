...
export default class ExpiredInventory extends Component {
    ...
    exportPDF = (columns) => {
        ...
        const data = this.state.outPutList.map(ele => [ele.planningUnit.id, getLabelText(ele.planningUnit.label), formatter(roundARU(ele.expiredQty,1),0), ele.batchInfo.batchNo, ele.batchInfo.autoGenerated == true ? i18n.t('static.program.yes') : i18n.t('static.program.no'), this.dateformatter(ele.batchInfo.createdDate), ele.shelfLife, this.dateformatter(ele.batchInfo.expiryDate), (formatter(Math.round(ele.cost * ele.expiredQty),0))]);
        let content = {
