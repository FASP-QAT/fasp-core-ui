...
export default class ExpiredInventory extends Component {
    ...
    exportCSV = (columns) => {
        ...
        this.state.outPutList.map(ele => A.push(addDoubleQuoteToRowContent([ele.planningUnit.id, (getLabelText(ele.planningUnit.label).replaceAll(',', ' ')).replaceAll(' ', '%20'), formatter(Number(ele.expiredQty).toFixed(3), 0), ele.batchInfo.batchNo, ele.batchInfo.autoGenerated == true ? i18n.t('static.program.yes') : i18n.t('static.program.no'), (dateFormatterCSV(ele.batchInfo.createdDate)).replaceAll(' ', '%20'), formatter(ele.shelfLife, 0), (this.dateFormatterCSV(ele.batchInfo.expiryDate)).replaceAll(' ', '%20'), (formatter(Math.round(ele.cost * ele.expiredQty), 0)).replaceAll(' ', '%20')])));
    ...
    exportPDF = (columns) => {
        ...
        const data = this.state.outPutList.map(ele => [ele.planningUnit.id, getLabelText(ele.planningUnit.label), formatter(Number(ele.expiredQty).toFixed(3), 0), ele.batchInfo.batchNo, ele.batchInfo.autoGenerated == true ? i18n.t('static.program.yes') : i18n.t('static.program.no'), this.dateformatter(ele.batchInfo.createdDate), ele.shelfLife, this.dateformatter(ele.batchInfo.expiryDate), (formatter(Math.round(ele.cost * ele.expiredQty), 0))]);
        let content = {
