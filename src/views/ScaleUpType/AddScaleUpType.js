...
const validationSchema = function (values) {
    return Yup.object().shape({
        label: Yup.string()
            ...
            .required(i18n.t('static.scaleUpType.scaleUpTypetext'))
    })
}
...
export default class AddScaleUpTypeComponent extends Component {
    ...
    dataChange(event) {
        let { scaleUpType } = this.state
        if (event.target.name === "label") {
            scaleUpType.label.label_en = event.target.value
        }
        this.setState(
            {
                scaleUpType
    ...
    Capitalize(str) {
        this.state.scaleUpType.label.label_en = str.charAt(0).toUpperCase() + str.slice(1)
    }
                        ...
                        <Card>
                            ...
                            <Formik
                                ...
                                onSubmit={(values, { setSubmitting, setErrors }) => {
                                    ...
                                    console.log(this.state.scaleUpType)
                                        ...
                                        handleReset
                                            ...
                                            <Form className="needs-validation" onSubmit={handleSubmit} onReset={handleReset} noValidate name='simpleForm' autocomplete="off">
                                                <CardBody style={{ display: this.state.loading ? "none" : "block" }}>
                                                    ...
                                                    <FormGroup>
                                                        <Label for="label">{i18n.t('static.scaleUpType.scaleUpType')}<span class="red Reqasterisk">*</span></Label>
                                                        <Input type="text"
                                                            name="label"
                                                            id="label"
                                                            bsSize="sm"
                                                            valid={!errors.label && this.state.scaleUpType.label.label_en != ''}
                                                            invalid={touched.label && !!errors.label}
                                                            onChange={(e) => { handleChange(e); this.dataChange(e); this.Capitalize(e.target.value) }}
                                                            onBlur={handleBlur}
                                                            value={this.state.scaleUpType.label.label_en}
                                                            required />
    ...
    cancelClicked() {
        this.props.history.push(`/scaleUpType/listScaleUpType/` + 'red/' + i18n.t('static.message.cancelled', { entityname }))
    }

    resetClicked() {
        let { scaleUpType } = this.state
        scaleUpType.label.label_en = '';
