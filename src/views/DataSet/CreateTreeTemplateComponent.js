    ...
    return Yup.object().shape({
        ...
        nodeUnitId: Yup.string()
            .test('nodeUnitId', i18n.t('static.validation.fieldRequired'),
                function (value) {
                    if ((parseInt(document.getElementById("nodeTypeId").value) == 3 || parseInt(document.getElementById("nodeTypeId").value) == 2 || parseInt(document.getElementById("nodeTypeId").value) == 4 || parseInt(document.getElementById("nodeTypeId").value) == 6) && document.getElementById("nodeUnitId").value == "") {
                        return false;
                    } else {
                        return true;
                    }
        ...
        nodeValue: Yup.string()
            .test('nodeValue', 'Please enter a valid number having less than or equal to 16 digits.',
                function (value) {
...
export default class CreateTreeTemplate extends Component {
    constructor(props) {
        ...
        this.changeShowConnections = this.changeShowConnections.bind(this);
    }
    /**
    ...
    exportPDF = () => {
        ...
        for (var i = 0; i < items1.length; i++) {
            ...
            e.text = text;
            e.flatList = this.state.treeTemplate.flatList;
            e.treeId = this.state.treeTemplate.treeTemplateId;
        ...
        doc
            ...
            .fontSize(12)
            .font('Helvetica')
            .text(i18n.t('static.forecastMethod.forecastMethod') + ': ' + document.getElementById("forecastMethodId").selectedOptions[0].text, 30, 85);
        doc
            .fillColor('#002f6c')
            .fontSize(12)
            .font('Helvetica')
            .text(i18n.t('static.listTreeTemp.templateName') + ': ' + this.state.treeTemplate.label.label_en, 30, 100);
        doc
            .fillColor('#002f6c')
            .fontSize(12)
            .font('Helvetica')
            .text(i18n.t('static.program.monthsInPast') + ': ' + document.getElementById("monthsInPast").value, 30, 115);
        doc
            .fillColor('#002f6c')
            .fontSize(12)
            .font('Helvetica')
            .text(i18n.t('static.program.monthsInFuture') + ': ' + document.getElementById("monthsInFuture").value, 30, 130);
    ...
    updateState(parameterName, value) {
        this.setState({
            ...
            if (parameterName == 'nodeId' && (value != null && value != 0)) {
                ...
                this.setState({ items, isChanged: false })
            }
    ...
    /**
     * Validation while creating number node and submitting data from modeling tab
     */
    ...
    acceptValue1() {
        ...
        if (this.state.currentItemConfig.context.payload.nodeType.id > 2) {
            ...
            if (this.state.currentModelingType == 2) {
                ...
                elInstance.setValueFromCoords(7, this.state.currentRowIndex, '', true);
                elInstance.setValueFromCoords(9, "", true);
                elInstance.setValueFromCoords(14, this.state.currentRowIndex, 0, true);
    ...
    callJexcelBuildFuntion() {
        ...
        for (var i = 0; i < reversedList.length - 1; i++) {
            ...
            var data = []
            var stopDt = map.get("8");
            var data2 = (i == 0) ? (map.get("8") - 6) : stopDt;
    ...
    buildMomJexcelPercent() {
        ...
        for (var j = 0; j < momList.length; j++) {
            var tempFuNode = (this.state.currentItemConfig.context.payload.nodeDataMap[0])[0].fuNode;
            data = [];
            ...
            if (this.state.currentItemConfig.context.payload.nodeType.id == 4 && tempFuNode.oneTimeUsage.toString() == "false" && tempFuNode.oneTimeDispensing != undefined && tempFuNode.oneTimeDispensing != null && tempFuNode.oneTimeDispensing.toString() != "" && tempFuNode.oneTimeDispensing.toString() == "false") {
                ...
                var f = momListParent.filter(c => c.month > (c.month <= 0 ? momList[j].month - tempMonth - 1 : momList[j].month - tempMonth) && c.month <= momList[j].month);
                f.map((item, index) => {
            ...
            data[9] = (this.state.currentItemConfig.context.payload.nodeDataMap[0])[0].nodeDataId
            ...
            data[13] = this.state.currentItemConfig.context.payload.nodeType.id == 4 ? ((this.state.currentItemConfig.context.payload.nodeDataMap[0])[0].fuNode.usageType.id == 2 ? Number(fuPerMonth).toFixed(4) : (tempFuNode.oneTimeUsage.toString() == "false" && tempFuNode.oneTimeDispensing != undefined && tempFuNode.oneTimeDispensing != null && tempFuNode.oneTimeDispensing.toString() != "" && tempFuNode.oneTimeDispensing.toString() == "false" ? tempNConvertToMonth * tempFuNode.usageFrequency * tempFuNode.noOfForecastingUnitsPerPerson : this.state.noFURequired)) : 1;
            ...
            if (this.state.currentItemConfig.context.payload.nodeType.id == 5 && parentNodeNodeData.fuNode.usageType.id == 2) {
                ...
                if (Math.floor(j / monthsPerVisit) == 0) {
                    ...
                    dataValue = dataArray[j - monthsPerVisit][16] + (j == 0 ? calculatedValueForCurMonth - patients : calculatedValueForCurMonth - calculatedValueForPrevMonth)
                }
    ...
    buildMomJexcel() {
        ...
        var options = {
            ...
            columns: [
                ...
                {
                    title: getLabelText(this.state.currentItemConfig.context.payload.label, this.state.lang) + " " + i18n.t('static.tree.monthlyEndNoSeasonality'),
                    type: this.state.currentItemConfig.context.payload.nodeType.id != 6 ? 'hidden' : 'numeric',
                    mask: '#,##0.00', decimal: '.',
                    readOnly: true
                },
                {
    ...
    getNodeTypeFollowUpList(nodeTypeId) {
        ...
        this.state.treeTemplate.flatList.filter(t => t.payload.downwardAggregationAllowed).map(t => downwardAggregationList.push({ label: t.payload.label.label_en, value: this.state.treeTemplate.treeTemplateId + "~" + "0" + "~" + t.payload.nodeId }));
        // for(var i = 0; i < this.state.treeTemplate.length; i++) {
        //     for(var j = 0; j < this.state.treeTemplate[i].scenarioList.length; j++) {
        //         if(this.state.treeTemplate[i].scenarioList[j].active) {
        //             tempDownwardAggregationList.filter(x => x.value.split("~")[0] == this.state.treeTemplate[i].treeId).map(x => downwardAggregationList.push({
        //                 label: x.label.split("~")[0]+" > "+this.state.treeTemplate[i].scenarioList[j].label.label_en+" > "+x.label.split("~")[1],
        //                 value: x.value.split("~")[0]+"~"+this.state.treeTemplate[i].scenarioList[j].id+"~"+x.value.split("~")[1]
        //             }))
        //         }
        //     }
        // }
    ...
    nodeTypeChange(value) {
        ...
        if (nodeTypeId == 1) {
            ...
            this.setState({
                ...
                (currentItemConfig.context.payload.nodeDataMap[0])[0].dataValue = 0;
                (currentItemConfig.context.payload.nodeDataMap[0])[0].fuNode = null;
                (currentItemConfig.context.payload.nodeDataMap[0])[0].puNode = null;
            });
    ...
    onCursoChanged(event, data) {
        ...
        if (item != null) {
            ...
            this.state.treeTemplate.flatList.map(f => f.payload.downwardAggregationList ? (f.payload.downwardAggregationList.map(da => (da.nodeId == data.context.payload.nodeId) ? sourceNodeUsageList.push({ treeId: this.state.treeTemplate.treeTemplateId, scenarioId: da.scenarioId, nodeId: da.nodeId, treeName: "", scenarioName: "", nodeName: f.payload.label.label_en, }) : "")) : "");
            this.setState({
                                            ...
                                            <div className="row">
                                                ...
                                                <FormGroup className="col-md-6" style={{ display: this.state.currentItemConfig.context.payload.nodeType.id == 6 || (this.state.aggregationNode && this.state.currentItemConfig.context.payload.nodeType.id < 4) ? 'block' : 'none' }}>
                                                    ...
                                                    <Input
                                                        ...
                                                        onChange={(e) => { handleChange(e); this.dataChange(e) }}
                                                        required
                                                        disabled={!this.state.editable ? true : this.state.currentItemConfig.context.payload.nodeType.id > 3 && this.state.currentItemConfig.context.payload.nodeType.id != 6 && this.state.currentItemConfig.context.parent != null ? true : false}
                                        ...
                                        <div className="row">
                                            {(this.state.currentItemConfig.context.payload.nodeType.id == 5) &&
                                                <>
                                                    ...
                                                    <FormGroup className="col-md-4 PUNodemarginTop" style={{ display: this.state.aggregationNode ? 'block' : 'none' }}>
                                                        <Label htmlFor="currencyId">{i18n.t('static.tree.nodeValue')}<span class="red Reqasterisk">*</span> <i class="fa fa-info-circle icons pl-lg-2" id="Popover7_5" onClick={this.toggleNodeValue} aria-hidden="true" style={{ color: '#002f6c', cursor: 'pointer' }}></i></Label>
                                                        <Input type="text"
                                                            id="nodeValue"
                                                            name="nodeValue"
                                                            bsSize="sm"
                                                    ...
                                                    <FormGroup className="col-md-7">
                                                        <Label htmlFor="currencyId">{i18n.t('static.ManageTree.ForecastingUnit')}</Label>
                                                        <Input type="text"
                                            ...
                                            {(this.state.currentItemConfig.context.payload.nodeType.id == 5) &&
                                                <>
                                                    ...
                                                    <FormGroup className="col-md-2">
                                                        <Label htmlFor="currencyId">{i18n.t('static.tree.conversionFUPU')} <i class="fa fa-info-circle icons pl-lg-2" id="Popover9" onClick={this.toggleConversionFactorFUPU} aria-hidden="true" style={{ color: '#002f6c', cursor: 'pointer' }}></i></Label>
                                                        <Input type="text"
                                                            id="conversionFactor"
                                                    ...
                                                    {(this.state.currentItemConfig.parentItem.payload.nodeDataMap[0])[0].fuNode.usageType.id == 2 &&
                                                        <>
                                                            <div style={{ display: "none" }}>
                                                                ...
                                                                <FormGroup className="col-md-2">
                                                                    <Label htmlFor="currencyId">{i18n.t('static.tree.consumptionInterval')}<i class="fa fa-info-circle icons pl-lg-2" id="Popover12" onClick={this.toggleQATEstimateForInterval} aria-hidden="true" style={{ color: '#002f6c', cursor: 'pointer' }}></i></Label>
                                                                </FormGroup>
                                                    ...
                                                    {((this.state.currentItemConfig.parentItem.payload.nodeDataMap[0])[0].fuNode.usageType.id == 1 || (this.state.currentItemConfig.parentItem.payload.nodeDataMap[0])[0].fuNode.usageType.id == 2) &&
                                                        <>
                                                            <div>
                                                            ...
                                                            <FormGroup className="col-md-6">
                                                                ...
                                                                <Label htmlFor="currencyId"># PU / {this.state.currentItemConfig.parentItem.payload.nodeDataMap[0][0].fuNode.usageType.id == 2 ? "Month / " : ""}{this.state.currentItemConfig.parentItem != null && this.state.currentItemConfig.parentItem.parent != null && this.state.unitList.filter(c => c.unitId == this.state.items.filter(x => x.id == this.state.currentItemConfig.parentItem.parent)[0].payload.nodeUnit.id).length > 0 && this.state.unitList.filter(c => c.unitId == this.state.items.filter(x => x.id == this.state.currentItemConfig.parentItem.parent)[0].payload.nodeUnit.id)[0].label.label_en}(s) (Calculated)</Label>
                                                                <Input type="text"
                                                                    id="puPerVisitQATCalculated"
                                                                    name="puPerVisitQATCalculated"
                                                                    readOnly={true}
                                                            ...
                                                            {this.state.currentItemConfig.parentItem != null && this.state.currentItemConfig.parentItem.payload.nodeDataMap[0][0].fuNode != null && (this.state.currentItemConfig.context.payload.nodeDataMap[0][0].puNode.sharePlanningUnit == "false" || this.state.currentItemConfig.context.payload.nodeDataMap[0][0].puNode.sharePlanningUnit == false) &&
                                                                <FormGroup className="col-md-6">
                                                                    <Label htmlFor="currencyId">{this.state.currentItemConfig.parentItem != null && (this.state.currentItemConfig.parentItem.payload.nodeDataMap[0])[0].fuNode != null && (this.state.currentItemConfig.parentItem.payload.nodeDataMap[0])[0].fuNode.usageType.id == 2 ? "# PU / Month / " : "# PU / "}{this.state.currentItemConfig.parentItem != null && this.state.currentItemConfig.parentItem.parent != null && this.state.unitList.filter(c => c.unitId == this.state.items.filter(x => x.id == this.state.currentItemConfig.parentItem.parent)[0].payload.nodeUnit.id).length > 0 && this.state.unitList.filter(c => c.unitId == this.state.items.filter(x => x.id == this.state.currentItemConfig.parentItem.parent)[0].payload.nodeUnit.id)[0].label.label_en}(s)</Label>
                                                                    <Input type="text"
                                                                        id="puPerVisit"
                                                                        name="puPerVisit"
                                                                        readOnly={!this.state.editable ? true : this.state.currentItemConfig.parentItem != null && this.state.currentItemConfig.parentItem.payload.nodeDataMap[0][0].fuNode != null && (this.state.currentItemConfig.context.payload.nodeDataMap[0][0].puNode.sharePlanningUnit == "false" || this.state.currentItemConfig.context.payload.nodeDataMap[0][0].puNode.sharePlanningUnit == false) ? false : true}
                                                                        bsSize="sm"
                                                                        valid={!errors.puPerVisit && this.state.currentItemConfig.context.payload.nodeType.id == 5 ? this.state.currentItemConfig.context.payload.nodeDataMap[0][0].puNode.puPerVisit != '' : !errors.puPerVisit}
                                                                        invalid={touched.puPerVisit && !!errors.puPerVisit}
                                                                        onBlur={handleBlur}
                                                                        ...
                                                                        value={this.state.currentItemConfig.parentItem != null && this.state.currentItemConfig.parentItem.payload.nodeDataMap[0][0].fuNode != null ? (this.state.currentItemConfig.context.payload.nodeDataMap[0][0].puNode.sharePlanningUnit == "false" || this.state.currentItemConfig.context.payload.nodeDataMap[0][0].puNode.sharePlanningUnit == false) ?
                                                                            addCommasWith8Decimals(this.state.currentItemConfig.context.payload.nodeDataMap[0][0].puNode.puPerVisit) :
                                                                            addCommasWith8Decimals(this.state.noFURequired / this.state.conversionFactor) : ''}>
                                                                    </Input>
                                                                    <FormFeedback className="red">{errors.puPerVisit}</FormFeedback>
                                                            ...
                                                            && this.state.planningUnitList.map((item, i) => {
                                                                return (
                                                                    <option key={i} value={item.planningUnitId}>
                                                                        {getLabelText(item.label, this.state.lang) + " | " + item.planningUnitId + " (" + i18n.t("static.tree.conversionFUToPU") + " = " + item.multiplier + ")"}
                                                                    </option>
                                                                )
                                                            }, this)}
                                                    </Input>
                                            ...
                                            <FormGroup className="col-md-12" style={{ display: this.state.currentItemConfig.context.payload.nodeType.id == 4 && this.state.currentItemConfig.context.payload.nodeDataMap != "" && this.state.currentItemConfig.context.payload.nodeDataMap[0][0].fuNode.usageType.id == 1 && this.state.currentItemConfig.context.payload.nodeDataMap[0][0].fuNode.oneTimeUsage != "true" && this.state.currentItemConfig.context.payload.nodeDataMap[0][0].fuNode.oneTimeUsage != true ? 'block' : 'none' }}>
                                                ...
                                                <FormGroup check inline>
                                                    <Input
                                                        ...
                                                        value={"true"}
                                                        checked={(this.state.currentItemConfig.context.payload.nodeType.id == 4 && this.state.currentItemConfig.context.payload.nodeDataMap != "" && this.state.currentItemConfig.context.payload.nodeDataMap[0][0].fuNode.usageType.id == 1 && this.state.currentItemConfig.context.payload.nodeDataMap[0][0].fuNode.oneTimeUsage.toString() != "true" ? (this.state.currentItemConfig.context.payload.nodeDataMap[0][0].fuNode.oneTimeDispensing != undefined && this.state.currentItemConfig.context.payload.nodeDataMap[0][0].fuNode.oneTimeDispensing != null && this.state.currentItemConfig.context.payload.nodeDataMap[0][0].fuNode.oneTimeDispensing.toString() != "" ? this.state.currentItemConfig.context.payload.nodeDataMap[0][0].fuNode.oneTimeDispensing.toString() : "true") : "") == "true" ? true : false}
                                                        onChange={(e) => {
                                                            this.dataChange(e)
                                                        }}
                                                    />
                                ...
                                <fieldset style={{ width: '100%' }} className="scheduler-border">
                                    ...
                                    <div className='col-md-12' style={{ display: this.state.targetSelect != 1 ? 'block' : 'none' }}>
                                        <div className="row">
                                            ...
                                            <FormGroup className="col-md-5">
                                                <Label htmlFor="currencyId">{this.state.currentItemConfig.context.payload.nodeType.id > 2 ? 'Change (% points)' : i18n.t('static.tree.targetChangePer')}<span class="red Reqasterisk">*</span>  <i class="fa fa-info-circle icons pl-lg-2" id="Popover23" onClick={this.toggleTargetChangePercent} aria-hidden="true" style={{ color: '#002f6c', cursor: 'pointer' }}></i></Label>
                                                <Input type="text"
    ...
    exportDoc() {
        ...
        dataArray.push(new Paragraph({
            children: [new TextRun({ "text": i18n.t('static.listTreeTemp.templateName') + " : ", bold: true }), new TextRun({ "text": document.getElementById("treeName").value })],
            spacing: {
                after: 150,
            },
        }));
    ...
    render() {
        ...
        const Node = ({ itemConfig, isDragging, connectDragSource, canDrop, isOver, connectDropTarget }) => {
            ...
            var outerLink = false;
            var itemConfigParent = this.state.treeTemplate.flatList.filter(x => x.id == itemConfig.parent);
            ...
            this.state.treeTemplate.flatList.map(f => f.payload.downwardAggregationList ? (f.payload.downwardAggregationList.map(da => (da.nodeId == itemConfig.payload.nodeId) ? sourceNodeUsageListCount.push({ treeId: this.state.treeTemplate.treeTemplateId, scenarioId: da.scenarioId, nodeId: da.nodeId, treeName: "", scenarioName: "", nodeName: f.payload.label.label_en, }) : "")) : "");
            return connectDropTarget(connectDragSource(
                (itemConfig.expanded ?
                    ...
                    :
                    <div className={(itemConfig.payload.nodeDataMap[0] != undefined && itemConfig.payload.nodeDataMap[0][0].isPUMappingCorrect == 0) || illegalNode ? "ContactTemplate boxContactTemplate contactTemplateBorderRed" : "ContactTemplate boxContactTemplate"} title={itemConfig.payload.nodeDataMap[0][0].notes}>
        ...
        return <div className="animated fadeIn">
                        ...
                        <div className="Card-header-reporticon pb-lg-0">
                            <div className="card-header-actions col-md-12 pl-lg-0 pr-lg-0 pt-lg-0">
                                <a className="pr-lg-0 pt-lg-0 float-left">
                                    <span style={{ cursor: 'pointer' }} onClick={this.cancelClicked}><i className="cui-arrow-left icons" style={{ color: '#002F6C', fontSize: '13px' }}></i> <small className="supplyplanformulas">{i18n.t('static.treeTemplate.returnToList')}</small></span>
                                </a>
                                        ...
                                        for (var i = 0; i < items.length; i++) {
                                            ...
                                            var json = {
                                                ...
                                                payload: {
                                                    ...
                                                    downwardAggregationAllowed: item.payload.downwardAggregationAllowed,
                                                    downwardAggregationList: item.payload.downwardAggregationList,
                                                    ...
                                                    <CardBody className="pt-0 pb-0" style={{ display: this.state.loading ? "none" : "block" }}>
                                                        <div className="col-md-12 pl-lg-0">
                                                            <Row>
                                                                ...
                                                                <FormGroup className="col-md-3 pl-lg-0">
                                                                    <Label htmlFor="languageId">{i18n.t('static.listTreeTemp.templateName')}<span class="red Reqasterisk">*</span></Label>
                                                                    <Input
                                                                        type="text"
                                                                        name="treeName"
                                                                        id="treeName"
                                                                ...
                                                                <FormGroup className="col-md-3 pl-lg-0" style={{ marginBottom: '0px' }}>
                                                                    <Label htmlFor="languageId">{i18n.t('static.forecastMethod.forecastMethod')}<span class="red Reqasterisk">*</span></Label>
                                                                    <Input
                                                                        type="select"
                                                                ...
                                                                <FormGroup className="col-md-3 pl-lg-0">
                                                                    <Label htmlFor="languageId">{i18n.t('static.program.monthsInPast')}<span class="red Reqasterisk">*</span> <i class="fa fa-info-circle icons pl-lg-2" id="Popover26" onClick={this.toggleMonthInPast} aria-hidden="true" style={{ color: '#002f6c', cursor: 'pointer' }}></i></Label>
                                                                    <Input
                                                                        type="number"
                                                                        name="monthsInPast"
                                                                        id="monthsInPast"
                                                                ...
                                                                <FormGroup className="col-md-3 pl-lg-0">
                                                                    <Label htmlFor="languageId">{i18n.t('static.program.monthsInFuture')}<span class="red Reqasterisk">*</span> <i class="fa fa-info-circle icons pl-lg-2" id="Popover27" onClick={this.toggleMonthInFuture} aria-hidden="true" style={{ color: '#002f6c', cursor: 'pointer' }}></i></Label>
                                                                    <Input
                                                                        type="number"
                                                                        name="monthsInFuture"
                                                                        id="monthsInFuture"
                                                                ...
                                                                <FormGroup className="col-md-6 pl-lg-0">
                                                                    <Label htmlFor="languageId">{i18n.t('static.program.notes')}</Label>
                                                                    <Input
                                                                        type="textarea"
                                                                        name="templateNotes"
                                                                        id="templateNotes"
                                                                ...
                                                                <FormGroup className="col-md-3 pl-lg-0 MarginTopMonthSelector">
                                                                    <Label htmlFor="languageId">{i18n.t('static.treeTemplate.monthSelector')}</Label>
                                                                    <Input
                                                                        type="select"
                                                                        name="monthId"
                                                                        id="monthId"
            ...
            <Modal isOpen={this.state.openAddNodeModal}
                ...
                <ModalHeader className="modalHeaderSupplyPlan hideCross">
                    <strong>{i18n.t('static.tree.Add/EditNode')}</strong>     {<div className="HeaderNodeText"> {
