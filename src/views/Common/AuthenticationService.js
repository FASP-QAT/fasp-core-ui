    ...
    setupAxiosInterceptors() {
        if (localStorage.getItem('curUser') != null && localStorage.getItem('curUser') != "") {
            ...
            if (temp_time_token > 21000000) {
                axios.get(`${API_URL}/refresh`, { headers: { Authorization: 'Bearer ' + decryptedToken } }).then(response => {
                    ...
                    let newDecryptedToken = CryptoJS.AES.decrypt(response.data.token.toString().replaceAll("Bearer ", ""), `${SECRET_KEY}`).toString(CryptoJS.enc.Utf8);
                    axios.defaults.headers.common['Authorization'] = 'Bearer ' + newDecryptedToken;
                }).catch(error => {
            ...
            if (localStorage.getItem('curUser') != null && localStorage.getItem('curUser') != '') {
                ...
                switch (route) {
                    ...
                    case "/ApplicationDashboard":
                        ...
                        break;
                    case "/report/supplyPlanScoreCard":
                        if (bfunction.includes("ROLE_BF_APPLICATION_DASHBOARD")) {
                            return true;
                        }
                        break;
