...
// export default class UsagePeriodListComponent extends Component {
...
//             loading: true
//         }
//         this.addNewUsagePeriod = this.addNewUsagePeriod.bind(this);
//         this.editUsagePeriod = this.editUsagePeriod.bind(this);
...
//     buildJexcel() {
//         let usagePeriodList = this.state.selSource;
//         // console.log("usagePeriodList---->", usagePeriodList);
//         let usagePeriodArray = [];
//         let count = 0;

//         for (var j = 0; j < usagePeriodList.length; j++) {
//             data = [];
//             data[0] = usagePeriodList[j].usagePeriodId
//             data[1] = getLabelText(usagePeriodList[j].label, this.state.lang)
//             data[2] = usagePeriodList[j].conversionFactor
//             data[3] = usagePeriodList[j].lastModifiedBy.username;
//             data[4] = (usagePeriodList[j].lastModifiedDate ? moment(usagePeriodList[j].lastModifiedDate).format(`YYYY-MM-DD`) : null)
//             usagePeriodArray[count] = data;
//             count++;
//         }
...
//             }.bind(this),
//         };
//         var usagePeriodListEl = jexcel(document.getElementById("tableDiv"), options);
//         this.el = usagePeriodListEl;
//         this.setState({
//             usagePeriodListEl: usagePeriodListEl, loading: false
//         })
//     }
//     hideSecondComponent() {
//         setTimeout(function () {
...
//                 if (AuthenticationService.getLoggedInUserRoleBusinessFunctionArray().includes('ROLE_BF_EDIT_INTEGRATION')) {
//                     this.props.history.push({
//                         pathname: `/usagePeriod/editUsagePeriod/${this.el.getValueFromCoords(0, x)}`,
//                         // state: { role }
//                     });
//                 }
//             }
...
//     addNewUsagePeriod() {
//         if (isSiteOnline()) {
//             this.props.history.push(`/usagePeriod/addUsagePeriod`)
//         } else {
...
//                                 {AuthenticationService.getLoggedInUserRoleBusinessFunctionArray().includes('ROLE_BF_ADD_INTEGRATION') && <a href="javascript:void();" title={i18n.t('static.common.addEntity', { entityname })} onClick={this.addNewUsagePeriod}><i className="fa fa-plus-square"></i></a>}
//                             </div>
//                         </div>
    ...
    buildJexcel() {
        ...
        if (papuList.length != 0) {
            for (var j = 0; j < papuList.length; j++) {
                ...
                data[2] = papuList[j].conversionFactor
        ...
        if (x == 2) {
            ...
            if (value == "") {
                ...
                if (!(reg.test(value))) {
                    this.el.setStyle(col, "background-color", "transparent");
                    this.el.setStyle(col, "background-color", "yellow");
                    this.el.setComments(col, i18n.t('static.planningUnit.conversionFactor'));
                } else {
